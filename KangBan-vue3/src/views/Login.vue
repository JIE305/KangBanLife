    ...
    <div class="max-w-md w-full space-y-8 relative z-10">
      <div class="text-center">
        <div
          ...
          <iconify-icon icon="solar:health-bold" width="32" class="text-white"></iconify-icon>
        </div>
        <h2 class="text-3xl font-bold text-slate-800">欢迎回来</h2>
        <p class="mt-2 text-slate-500">登录你的康伴生活账号</p>
      </div>
      ...
      <form class="bg-white/95 backdrop-blur-sm rounded-3xl shadow-xl p-8 space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="flex items-center justify-between">
          <label class="flex items-center gap-2">
            <input type="checkbox" v-model="form.rememberMe" @change="userStore.setRememberMe(form.rememberMe)"
              class="w-4 h-4 text-emerald-500 rounded border-slate-300 focus:ring-emerald-500" />
            <span class="text-sm text-slate-600">记住我</span>
          </label>
          <a href="#" class="text-sm text-emerald-600 hover:text-emerald-500 font-medium">忘记密码?</a>
        </div>
      ...
      <div class="relative">
        ...
        <div class="relative flex justify-center text-sm">
          <span class="px-4 bg-gradient-to-br from-emerald-50 via-white to-blue-50 text-slate-500">或</span>
        </div>
      </div>

      <div class="flex gap-4">
        <button
          class="flex-1 py-3 border border-slate-200 rounded-xl flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
          <iconify-icon icon="solar:phone-linear" width="20" class="text-green-500"></iconify-icon>
          <span class="text-sm font-medium text-slate-700">手机号登录</span>
        </button>
        <button
          class="flex-1 py-3 border border-slate-200 rounded-xl flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
          <iconify-icon icon="solar:message-circle-linear" width="20" class="text-green-600"></iconify-icon>
          <span class="text-sm font-medium text-slate-700">微信登录</span>
        </button>
      </div>

      <p class="text-center text-sm text-slate-600">
        还没有账号？
        <router-link to="/register" class="font-medium text-emerald-600 hover:text-emerald-500">前往注册</router-link>
      </p>
    </div>
  </div>
</template>
